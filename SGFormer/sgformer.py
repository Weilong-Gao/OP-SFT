...
class SGFormer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[96, 192, 384, 768],
                 num_heads=[4, 6, 12, 24], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=[4, 6, 24, 2], sr_ratios=[8, 4, 2, 1], num_stages=4, linear=False,):
        super().__init__()
...
@register_model
def sgformer_b(pretrained=True, **kwargs):
    model = SGFormer(
